...
getchar();
	...
	ifstream querylist("query_list.txt",ios::in);
	string class_name, file_name;
	float sum = 0;
	while (querylist >> class_name >> file_name)
	{
		//string gtq = "oxford_ground_truth/all_souls_1";
		//string gtq = argv[1];

		vector<string> ranked_list = load_list("data2/"+file_name+".rank");
		set<string> good_set = vector_to_set(load_list("oxford_ground_truth/" +class_name+ "_good.txt"));
		set<string> ok_set = vector_to_set(load_list("oxford_ground_truth/" + class_name + "_ok.txt"));
		set<string> junk_set = vector_to_set(load_list("oxford_ground_truth/" + class_name + "_junk.txt"));
