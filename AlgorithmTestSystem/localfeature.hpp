...
class LocalFeature{
public:
	static vector<float> genDescriptorReduced(Mat& descriptors, const Mat& mlModel)
	{
		Mat dmat;
		do_metric(mlModel, descriptors, dmat);
		return getVector(dmat);
	}
	static void extDenseVlSiftDes(Mat& sImg, Mat& descriptors)
	{
	...
	static void extSparseVlSiftDes(Mat &sImg, Mat& descriptors)
	{
